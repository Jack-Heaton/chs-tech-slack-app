<script lang="ts">
  ...
  let { checklistItems = $bindable<ChecklistItem[]>([]) , backUrl = '/tech-slack', nextUrl = '/tech-slack/join' } = $props();
...
<form action="/bot-trap" method="POST" class="flex flex-col gap-6">
	...
	<div class="flex flex-row justify-between gap-6 text-3xl/tight font-bold">
		<LinkButton href={backUrl}>Back</LinkButton>
		<LinkButton href={`${nextUrl}?req=${reqUrl()}`}>Next</LinkButton>
		<LinkButton type="submit" component="button" tabIndex={-1} class="hidden">Submit for Bots Don't Click This</LinkButton>
	</div>
</form>